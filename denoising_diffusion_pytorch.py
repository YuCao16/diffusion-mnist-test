...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        eps: float = 1e-5 if x.dtype == torch.float32 else 1e-3
        var: torch.Tensor = torch.var(x, dim=1, unbiased=False, keepdim=True)
...
class Unet(nn.Module):
    def __init__(
        ...
        if self.random_or_learned_sinusoidal_cond:
            ...
            sinu_pos_emb = SinusoidalPosEmb(dim)  # dim -> dim x dim
            fourier_dim = dim
        ...
        self.final_conv = nn.Conv2d(dim, self.out_dim, 1)  # self.out_dim = 3 or 6
    ...
    def forward(self, x, time, x_self_cond=None):
        ...
        x = self.init_conv(x)  # b x channel x dim x dim -> b x 64 x dim x dim
        r = x.clone()

        t = self.time_mlp(time)  # time -> (dim x 4) x (dim x 4)
        ...
        for block1, block2, attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)  # shape: (b, 768, h, w)
            x = block1(x, t)
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert type(self) != GaussianDiffusion or model.channels == model.out_dim
    ...
    def predict_start_from_noise(self, x_t, t, noise):  # formula 115
        """
        return x_0
        """
        return (
    ...
    def predict_noise_from_start(self, x_t, t, x0):  # transformation of formula 69
        """
        return \epsilon_0
        """
        return (
    ...
    def p_mean_variance(self, x, t, x_self_cond=None, clip_denoised=True):
        ...
        if clip_denoised:
            x_start.clamp_(-1.0, 1.0)  # x > 1 = 1, x < -1 = -1, otherwise x = x
    ...
    def ddim_sample(self, shape, clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step"):
            # sampling #batch samples at a time
    ...
    def sample(self, batch_size=16):
        ...
        return (self.ddim_sample if self.is_ddim_sampling else self.p_sample_loop)(
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        model_out = self.model(x, t, x_self_cond)  # output of Unet
        ...
        # if not use p2_loss_weight, we can change reduction to "mean" and remove below codes
        loss = reduce(loss, "b ... -> b (...)", "mean")

        loss = loss * extract(self.p2_loss_weight, t, loss.shape)  # identity
        return loss.mean()
    ...
    for i in range(1, 11):
        ...
        for k in dataset:
            if k[1] == i:
                ...
                if j == 0:
                    break
    image_list = [(i.convert("RGB"),j) for (i,j) in image_list]
    return image_list
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.accelerator.native_amp = amp  # If turn on Automatic mixed precision
        ...
        self.num_samples = num_samples  # number of image displayed in evaluation
        self.save_and_sample_every = save_and_sample_every

        self.batch_size = train_batch_size  # 16
        self.gradient_accumulate_every = gradient_accumulate_every  # 1

        self.train_num_steps = train_num_steps  # 100k
        self.image_size = diffusion_model.image_size  # 128
        self.mnist = mnist
        ...
        # optimizer (fg = "#febd69", bg = "#131921"lr=1e-4, betas=(0.9, 0.99))
